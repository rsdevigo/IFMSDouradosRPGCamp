...
//=============================================================================
 /*:
 * @plugindesc v1.07 (É necessário ter YEP_SkillCore.js) Cooldowns podem
 * ser aplicados para habilidades.
 ...
 * @desc Esse é o formato de texto usado para cooldowns.
 * %1 - Rodadas Faltando
 * @default %1CD
 *
 * @param Cooldown Font Size
 * @desc Esse é o tamanho da fonte usado para cooldowns.
 * Padrão: 28
 * @default 20
 *
 * @param Cooldown Text Color
 * @desc Ajusta a cor de texto usada para cooldowns.
 * @default 6
 *
 * @param Cooldown Icon
 * @desc Qual ícone para ser usado em cooldowns.
 * Use 0 para nenhum ícone.
 * @default 75
 *
 * @param Cooldown After Battle
 * @desc Como são os cooldowns manuseados depois da batalha?
 * @default -10
 *
 * @param Cooldown Steps
 * @desc Fora da batalha, isso são quantos passos no mapa o jogador
 * tem que andar para drop cada cooldown por 1.
 * @default 5
 *
 * @param Cooldown Bypass
 * @desc  Isso é uma lista de habilidades que não podem estar no cooldown.
 * Então dessa forma, habilidades como Attack, Guard.
 ...
 *
 * @param Warmup Format
 * @desc Esse é o formato de texto usado para warmups.
 * %1 - Rodadas Faltando
 * @default %1WU
 *
 * @param Warmup Font Size
 * @desc Esse é o tamanho da fonte usado para warmups.
 * Padrão: 28
 * @default 20
 *
 * @param Warmup Text Color
 * @desc Ajusta a cor de texto usada para warmups.
 * @default 4
 *
 * @param Warmup Icon
 * @desc Qual ícone para ser usado em warmups.
 * Use 0 para nenhum ícone.
 * @default 75
 *
 * @param ---Battle Core---
 ...
 *
 * @param Time Based
 * @desc Se um sistema de batalha for baseado em Tick's, usar
 * tempo em vez de rodadas para CD? NO - false   YES - true
 * @default false
 *
 * @param Turn Time
 * @desc Quantos ticks devem passar para serem iguais a 1 CD?
 * @default 100
 *
 * @help
 * ============================================================================
 ...
 * ============================================================================
 *
 * Esse plugin necessita ter YEP_SkillCore.
 * Certifique-se que esse plugin esteja localizado abaixo do YEP_SkillCore na
 * lista de plugins.
 *
 * Esse plugin permite você a dar para suas habilidades os cooldowns.
 * Cooldowns são um limite forçado numa habilidade para prevenir que
 * elas sejam usadas constantemente.
 *
 * ============================================================================
 * Tipos de Cooldown
 * ============================================================================
 *
 * Cooldown (Padrão)
 * O cooldown padrão apenas ocorre se a habilidade tem um cooldown a
 * pagar. Quando usada, a habilidade não pode ser usada por x rodadas
 * como indicada pelo cooldown. Existem um número de coisas que
 * contribuem para que o cooldown desça. A primeira seria simplesmente
 * esperar. Cada rodada na batalha causa o cooldown a descer por 1
 * rodada. Habilidades podem ser usadas para acelerar esse processo.
 * A segunda seria terminar batalhas. Terminando uma batalha irá causar
 * todos os cooldowns a descer por uma certa quantia (pode ser definida
 * nos parâmetros). E a terceira seria andar no campo do mapa. Cada
 * quantidade certa de passos permitem que o cooldown de uma habilidade
 * diminua.
 *
 * Warmups
 * Warmups fazem a mesma coisa que Cooldowns: previnem habilidades de
 * serem usadas até que seu cronômetro termine. A diferença, porém, é
 * que warmups só ocorrem uma vez durante a batalha: bem no começo. Se
 * uma habilidade tem um cronômetro warmup, ela irá começar no momento
 * em que for para batalha e instântaneamente desaparecer depois da
 * batalha. Warmups não se empilham em cima de nenhum cooldown existente.
 * Se um cooldown já estiver ocorrendo quando uma habilidade está na fase
 * de warmup, tanto o warmup quanto o cooldown irão simultaneamente
 * update.
 *
 * Cooldowns Unido
 * Um cooldown unido ocorre quando uma habilidade que foi usada causa outra
 * habilidade na biblioteca de habilidade do proprietário a ter um cooldown.
 * Todos os outros atributos desse cooldown são os mesmos de um cooldown
 * padrão. Esse tipo de cooldown irá ter prioridade sobre Skill Type Cooldowns
 * e Global Cooldowns se esse valor for definido.
 *
 * Skill Type Cooldowns
 * Quando um Skill Type Cooldown ocorre, todas as habilidades atualmente
 * na biblioteca de habilidades do proprietário com seu correspondente
 * Skill Type vão ficar no cooldown. Todos os outros atributos desse
 * cooldown são os mesmos de um cooldown padrão. Esse tipo de cooldown
 * irá ter prioridade sobre Global Cooldowns se esse valor for definido.
 *
 * Quando um cooldown é aplicado para uma habilidade que já tem um cooldown,
 * o cooldown irá mudar para qualquer que for o valor maior. Isso significa
 * que se uma habilidade tiver 3 rodadas de cooldown e um Skill Type Cooldown
 * estabelecer para 1 rodada, as 3 rodadas permaneceriam. De outro lado, se
 * uma habilidade tiver 3 rodadas e o Skill Type Cooldown estabelecer para 5
 * rodadas, então o cooldown mudaria para 5 rodadas.
 *
 * ============================================================================
 * Notetags
 * ============================================================================
 *
 * Use os seguintes notetags para alterar as propriedades de cooldown de
 * uma habilidade.
 *
 * Notetags de Habilidade:
 *   <Cooldown: x>
 *   Estabelece o cooldown para a habilidade para X rodadas. Esse cooldown
 *   só afeta essa habilidade apenas. Esse valor terá prioridade sobre
 *   Skill Type Cooldowns e Global Cooldowns.
 *
 *   <After Battle Cooldown: +x>
 *   <After Battle Cooldown: -x>
 *   Depois que uma batalha terminar (vitória, derrota, ou escape), muda o
 *   cooldown dessa habilidade para +x rodadas ou -x rodadas.
 *
 *   <Cooldown Steps: x>
 *   Fora de batalha, cada x passos que um Jogador fizer, o cooldown
 *   dessa habilidade cairá em 1.
 *
 *   <Skill x Cooldown: y>
 *   Quando usando essa habilidade, depois de pagar pelo custo da habilidade,
 *   habilidade x terá um cooldown unido por y rodadas. Esse valor terá
 *   prioridade sobre Skill Type Cooldowns e Global Cooldowns.
 *
 *   <SType x Cooldown: y>
 *   Quando usando essa habilidade, depois de pagar pelo custo da habilidade,
 *   todas as habilidades com o correspondente Skill Type x terão um cooldown
 *   de y. Esse valor terá prioridade sobre Global Cooldowns.
 *
 *   <Global Cooldown: x>
 *   Quando usando essa habilidade, todas as habilidades dentro da área
 *   de biblioteca de habilidade do combatente estabelecem estar no
 *   cooldown por x rodadas. Esse valor tem menos prioridade que
 *   Individual Cooldowns e Skill Type Cooldowns.
 *
 *   <Bypass Cooldown>
 *   Isso causa a habilidade a evitar cooldowns, não importa o quê. Isso
 *   deve ser usado para habilidades do tipo Attack, Guard, Escape, etc.
 *   que não deveriam ter um cooldown atribuído a elas.
 *
 ...
 *   <Skill name Cooldown: -y>
 *   Alvos acertados por essa habilidade terão o cooldown da habilidade
 *   x ajustada por y. Isso não se aplica ao usuário e aplica apenas
 *   para os alvos.
 *
 *   <SType x Cooldown: +y>
 *   <SType x Cooldown: -y>
 *   Alvos acertados por essa habilidade terão todas as habilidades em
 *   sua biblioteca de habilidades com Skill Type x a terem seus cooldowns
 *   ajustados por y. Isso não se aplica para o usuário e aplica apenas
 *   para os alvos.
 *
 *   <Global Cooldown: +x>
 *   <Global Cooldown: -x>
 *   Alvos acertados por essa habilidade terão todas as habilidades em
 *   sua biblioteca de habilidades a terem seus cooldowns ajustados por
 *   y. Isso não se aplica para o usuário e aplica apenas para os alvos.
 *
 * Notetags de Personagem, Classe, Inimigo, Arma, Armadura, e Estado:
 *   <Skill x Cooldown Duration: y%>
 *   <Skill name Cooldown Duration: y%>
 *   Altera a duração de cooldown da habilidade x para y% quando o custo
 *   do cooldown for aplicado. Esse efeito se aplica apenas para
 *   habilidade x.
 *
 *   <SType x Cooldown Duration: y%>
 *   Altera a duração de cooldown de habilidades com Skill Type x com y%
 *   quando o custo de cooldown for aplicado. Esse efeito se aplica apenas
 *   para habilidade x.
 *
 *   <Global Cooldown Duration: x%>
 *   Altera a duração de cooldown de todas as habilidades para x% quando o
 *   cooldown for aplicado.
 *
 *   <Skill x Cooldown Rate: y%>
 *   <Skill name Cooldown Rate: y%>
 *   Estabelece o ritmo de cooldown para habilidade x para y% quando o
 *   contador de cooldown for para baixo. Esse efeito se aplica apenas
 *   para habilidade x.
 *
 *   <SType x Cooldown Rate: y%>
 *   Estabelece o ritmo de cooldown para habilidades Skill Type x para y%
 *   quando o contador de cooldown for para baixo. Esse efeito se aplica
 *   apenas para habilidades Skill Type x.
 *
 *   <Global Cooldown Rate: x%>
 *   Estabelece o ritmo de cooldown para todas as habilidades para x%
 *   quando o contador de cooldown for para baixo.
 *
 *   <Skill x Cooldown: +y>
 *   <Skill x Cooldown: -y>
 *   <Skill name Cooldown: +y>
 ...
 *
 * ============================================================================
 * Yanfly Engine Plugins - Comandos de Sequências de Ações
 * ============================================================================
 *
 * Se você tem YEP_BattleEngineCore.js instalado com esse plugin localizado
 * embaixo dele no Plugin Manager, você pode fazer uso dessas sequências de
 * ações extras relacionadas a cooldown.
 *
 *=============================================================================
 ...
 * GLOBAL COOLDOWN: targets, X
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * Estabelece o cooldown para todos os alvos para serem ajustados pelo valor
 * X. Isso se aplica a todas as habilidades que não evitam cooldowns.
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * Exemplo de uso: global cooldown: target, +5
 ...
 * SKILL X COOLDOWN: targets, Y
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * Causa a habilidade X a ser ajustada pelo valor Y para os alvos. Isso se
 * aplica apenas para o cooldown da específica habilidade x.
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * Exemplo de uso: skill 10 cooldown: target, +5
 ...
 * SKILL TYPE X COOLDOWN: targets, Y
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * Causa habilidades do tipo X a serem ajustadas pelo valor Y para os alvos.
 * Isso se aplica apenas para o cooldown da específica habilidade tipo X.
 *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * Exemplo de uso: skill type 1 cooldown: target, +5
